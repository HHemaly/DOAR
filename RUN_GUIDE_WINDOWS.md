...
keeps any accepted leakage override explicit and labels those runs exploratory.

- Repo root: `C:\Users\Ahmed\Documents\Codex\2026-07-19\it\DOAR\DOAR-main`
...
## 6. Leakage status and exploratory override
...
- **`leakage_status: FAIL_LEAKAGE_DETECTED`** → leaked images were quarantined and
  ...
  `--allow-leakage-override --override-justification "Known duplicates accepted
  for exploratory run"` only for the currently accepted exploratory runs.
  Overridden results must not be presented as final leakage-free thesis evidence.
...
```powershell
Re-run the same `compare-deep-models` command. Completed runs are skipped and
incomplete runs resume automatically from their own `last.pt`.
```
...
python main.py extract-embeddings --manifest "$OUT\manifest.csv" `
  --deep-comparison "$OUT\deep\deep_comparison.json" `
  --device auto --batch-size 4 --output "$OUT\emb_finetuned"
```
...
python main.py explain-gradcam --image "$DS\valid\Happy\<pick-one>.png" `
  --deep-comparison "$OUT\deep\deep_comparison.json" --output "$OUT\gradcam"
```
...
```powershell
python main.py predict-image --image "$DS\valid\Happy\<pick-one>.png" `
  --deep-comparison "$OUT\deep\deep_comparison.json"
python main.py analyze-image --image "$DS\valid\Happy\<pick-one>.png" `
  --deep-comparison "$OUT\deep\deep_comparison.json" --output "$OUT\case_001"
```

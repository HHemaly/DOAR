...
## Phase 3 — Detector layer — planning complete (2026-08-02), **implementation awaiting review**
...
Planning + validation prep delivered: `DETECTOR_DEPENDENCY_MATRIX.csv`, `PHASE3_DETECTOR_EVALUATION_PLAN.md` (candidate comparison, recommended sequence: shapes/symbols first, then face/eyes, then animals last; recommended first experiment: classical-CV circularity for `circles`, zero new dependency), `docs/PHASE3_ANNOTATION_SCHEMA.md`, and `src/doar/detectors/` (evaluation scaffolding only — no model, no download, not wired into any user-facing path). No detector has been implemented, downloaded, or enabled. **Stopped for review per user instruction before any implementation begins.**

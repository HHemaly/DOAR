...
class ThesisTests(unittest.TestCase):
    ...
    def test_nested_results_are_discovered(self):
        from doar.thesis import generate_thesis_outputs
        with tempfile.TemporaryDirectory() as d:
            _write(d, "deep/results/deep_comparison.json", {"leaderboard": [
                {"model": "small_cnn", "mean_valid_macro_f1": .5,
                 "std_valid_macro_f1": .01}]})
            res = generate_thesis_outputs(d)
            if res["generated"]:
                self.assertIn("deep/results/deep_comparison.json",
                              res["figures"][0]["source_experiment"].replace("\\", "/"))

...
cells = [
    ...
    md("## 7. Train (GPU recommended)\n",
       "Transfer model (ResNet18) by default. Use `--model baseline` for the\n",
       "small CNN, or `mobilenet` / `efficientnet` for comparison."),
    code("!python main.py train --out \"$OUTPUT\" --model transfer --epochs 25 --batch-size 32"),

    md("## 8. Evaluate on the untouched test split\n",
       "Confusion matrix, per-class metrics, predictions_test.csv, etc."),
    code("!python main.py evaluate --out \"$OUTPUT\""),
    code("import json\n",
    ...
    md("## 9. Analyze example drawings (interpretation pipeline)\n",
       "Objective features + rules + claims + validators + safe parent answer."),
    code("!python main.py analyze-dataset --data \"$DATASET\" --out \"$OUTPUT\" --max 3"),

    md("## 10. Collate thesis figures and tables"),

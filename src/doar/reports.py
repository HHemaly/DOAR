...
def render_report(analysis: dict, judges: dict, language: str = "en", parent: bool = False) -> str:
    ...
    return f"""<!doctype html><html lang="{language}" dir="{direction}"><head><meta charset="utf-8">
    ...
    table{{border-collapse:collapse;width:100%;margin-bottom:1rem}}th,td{{border:1px solid #ccc;padding:.45rem;text-align:start}}
    ...
    <p>{html.escape(disclaimer)}</p></body></html>"""
...
def save_reports(analysis: dict, judges: dict, output: Path) -> dict[str, str]:
    ...
    bilingual.write_text(
        reports["professional_en"] + "<hr>" + reports["professional_ar"], encoding="utf-8"
    )

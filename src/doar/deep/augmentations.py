...
def augmentation_ops(profile: str = "conservative"):
    """Return the PIL-space augmentation ops for a profile (before ToTensor).

    These are the drawing-safe augmentations inserted between resize/crop and
    tensor conversion + normalization, so a spec-driven training transform can
    reuse them (Item 1). The conservative profile is preserved unchanged."""

...
def run_deep_comparison(
    ...
    epochs: int = 30, grad_accum_steps: int = 4, calibration: str | None = None,
    trainer=None,
) -> dict:
    ...
    for model in models:
        for seed in seeds:
            ...
            completed = run_out / "training_result.json"
            if completed.exists() and (run_out / "best.pt").exists():
                result = json.loads(completed.read_text(encoding="utf-8"))
            else:
                resume = run_out / "last.pt"
                kwargs = {}
                if resume.exists():
                    kwargs["resume"] = str(resume)
                call_kwargs = {
                    "seed": seed, "epochs": epochs, "batch_size": batch_size,
                    "image_size": image_size, "device": device, "workers": 0,
                ...
                result = trainer(dataset, model, str(run_out), **call_kwargs)
    ...
    if summary["winner"]:
        eligible = [r for r in run_records if r["model"] == summary["winner"]]
        summary["winning_run"] = max(
            eligible, key=lambda r: float(r["best_valid_macro_f1"])
        )

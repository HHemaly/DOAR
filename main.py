...
def main() -> None:
    ...
    analyze.add_argument("--deep-comparison")
    ...
    evaluate.add_argument("--deep-comparison")
    ...
    image_train.add_argument("--batch-size", type=int, default=4)
    ...
    image_train.add_argument("--grad-accum-steps", type=int, default=4)
    ...
    image_predict.add_argument("--checkpoint")
    image_predict.add_argument("--deep-comparison")
    ...
    explain_cam.add_argument("--checkpoint")
    explain_cam.add_argument("--deep-comparison")
    ...
    deep_compare.add_argument("--grad-accum-steps", type=int, default=4)
    ...
    embedding.add_argument("--deep-comparison",
                           help="Use validation winner as the fine-tuned backbone")
    embedding.add_argument("--device", default="auto")
    embedding.add_argument("--batch-size", type=int, default=4)
    ...
    if args.command == "analyze-image":
        ...
        checkpoint = (resolve_checkpoint(args.emotion_checkpoint, args.deep_comparison)
                      if args.emotion_checkpoint or args.deep_comparison else None)
        result = analyze_image(args.image, args.output, checkpoint)
        ...
        from doar.deep.selection import resolve_checkpoint
        checkpoint = resolve_checkpoint(args.checkpoint, args.deep_comparison)
        print(json.dumps(evaluate_model(
            args.manifest, checkpoint, args.output, args.split, args.unlock_test,
        ...
        print(json.dumps(train_image_model(
            ...
            batch_size=_opt("batch_size", int, 4), image_size=_opt("image_size", int, 224),
            ...
            grad_accum_steps=_opt("grad_accum_steps", int, 4),
        ...
        print(json.dumps(predict_image(
            args.image, resolve_checkpoint(args.checkpoint, args.deep_comparison), args.device
        ), ensure_ascii=False, indent=2))
        ...
        print(json.dumps(generate_gradcam(
            args.image, resolve_checkpoint(args.checkpoint, args.deep_comparison),
            args.output, args.device), indent=2))
        ...
        if args.deep_comparison:
            from doar.deep.selection import load_winner
            args.backbone = f"finetuned:{load_winner(args.deep_comparison)['checkpoint']}"
        config = load_config(args.config, {
